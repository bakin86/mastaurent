...
const select = {
  ...
  district: true, addressLine: true, status: true, deliveryStatus: true, createdAt: true,
...
deliveriesRouter.get('/available', requireStaff, requireRole('DRIVER'), asyncHandler(async (req, res) => {
  const orders = await prisma.order.findMany({
    where: { tenantId: req.tenantId!, driverId: null, type: 'DELIVERY', deliveryStatus: 'READY_FOR_DELIVERY' },
...
const statusSchema = z.object({ status: z.enum(['PICKED_UP', 'ON_THE_WAY', 'DELIVERED']), latitude: z.number().min(-90).max(90), longitude: z.number().min(-180).max(180) });
deliveriesRouter.patch('/:id/status', requireStaff, requireRole('DRIVER'), asyncHandler(async (req, res) => {
  const { status, latitude, longitude } = statusSchema.parse(req.body);
